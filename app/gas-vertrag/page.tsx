...
export default function GasVertragPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    const q = query(collection(db, "gasContracts"), orderBy("createdAt", "desc"));
    const unsub = onSnapshot(
      q,
      (snap) => {
        const list: GasContract[] = snap.docs.map((d) => ({
          id: d.id,
          ...(d.data() as any),
        }));
        setContracts(list);
  ...
  function handleEdit(contract: GasContract) {
  ...
  async function handleSave() {
    if (!propertyName.trim() || !providerName.trim()) {
      setError("Bitte Objektname und Gasanbieter ausfüllen!");
      return;
    }
    ...
    try {
      ...
      if (pdfFile) {
        ...
        try {
          ...
          const fileRef = ref(storage, `gas-contracts/${fileName}`);
      ...
      if (selectedContract) {
        await updateDoc(doc(db, "gasContracts", selectedContract.id), data);
      } else {
        await addDoc(collection(db, "gasContracts"), {
          ...data,
          createdAt: serverTimestamp(),
          createdByUid: user.uid,
          createdByName: getUserDisplayName(user.email || ""),
  ...
  async function handleDelete(contract: GasContract) {
    if (!confirm(`Gasvertrag für "${contract.propertyName}" wirklich löschen?`)) {
      return;
    }
    ...
    try {
      ...
      await deleteDoc(doc(db, "gasContracts", contract.id));
      setSelectedContract(null);
      setIsEditing(false);
    } catch (error) {
  ...
  function generateCancellationPdf(contract: GasContract): void {
    ...
    doc.setFont("Helvetica", "normal");
    doc.setFontSize(11);
    doc.text(contract.providerName || "Gasanbieter", margin, y);
    y += 5;
    doc.setFontSize(10);
    ...
    y += 10;
    doc.setFontSize(11);
    const mainText = `hiermit kündigen wir unseren Gasversorgungsvertrag fristgerecht zum nächstmöglichen Zeitpunkt.
...
Ich beantrage die sofortige Beendigung der automatischen Abbuchungen von meinem Bankkonto. Bitte senden Sie mir eine schriftliche Bestätigung über den Erhalt dieser Kündigung sowie ggf. eine Abschlussrechnung.`;
    ...
    // Download PDF
    const fileName = `Kündigung_Gas_${contract.propertyName.replace(/\s+/g, '_')}_${Date.now()}.pdf`;
    doc.save(fileName);
  }
        ...
        <div className="flex items-start gap-4 mb-3">
          ...
          <div>
            ...
            <p className="mt-2 text-lg opacity-60">
              Verwaltung der Gasverträge für alle Immobilien
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="rounded-[28px] surface p-6 space-y-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-semibold">
                {selectedContract ? "Bearbeiten" : "Neuer Gasvertrag"}
              </h2>
              <button
                onClick={resetForm}
            ...
            <div className="space-y-4">
              ...
              {/* Anbieter */}
              <div>
                <label className="text-xs muted font-medium">Gasanbieter *</label>
                <input
                  type="text"
                  value={providerName}
              ...
              <div>
                <label className="text-xs muted font-medium">Zählerstand (m³)</label>
                <input
                  type="text"
                  value={meterReading}
                  ...
                  {selectedContract.meterReading && (
                    <div>
                      ...
                      <div className="text-lg font-semibold">
                        {selectedContract.meterReading} <span className="text-sm muted font-normal">m³</span>
                      </div>
                    </div>
                  )}
                </div>
